...
function drawWeekends() {
    ...
    $.get('/Weekends/ReadForDrawing', function (data) {
        ...
        data.forEach(function (item, i, arr) {
            holidays.push(kendo.toString(new Date(parseInt(item.Date.replace(/^\D+/g, ''))), 'm'));
        });
        if (view.table != null) {
            view.table.find("td[role='gridcell']").each(function () {
                if ($(this) != null) {
                    ...
                    if (element != null) {
                        ...
                        if (slot != null) {
                            var dateSlot = kendo.toString(slot.startDate, 'm');
                            if ($.inArray(dateSlot, holidays, 0) >= 0) {
        ...
        if (view.groups != null)
        {
            onDataBound(view._dates.length - holidayCounter / view.groups.length - weekendsAmount);
        }
    });
}
...
function onTaskAdd(e) {
    ...
    programmers.dataSource.read().then(function () {
...
function handleEditEvent(e) {
    ...
    excludeDeletedProgrammers();
    setEvents();
    drawWeekends();
    setRemoveLabels();
}

function excludeDeletedProgrammers() {
    var scheduler = $('#scheduler').data('kendoScheduler');
    var view = scheduler.view();
    console.log(view);
}
